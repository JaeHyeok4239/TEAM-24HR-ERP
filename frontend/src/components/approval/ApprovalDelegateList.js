...
export default function ApprovalDelegateList() {
  ...
  const selectedLine = lineList.find(
    (l) => String(l.approvalLineId) === String(form.approvalLineId),
  );
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const res = await apiRequest(
        `/api/approval/management/delegate?${params.toString()}`,
        { method: "GET" },
      );
  ...
  useEffect(() => {
    if (!showForm) return;
    const load = async () => {
      const res = await apiRequest("/api/approval/line?size=100", {
        method: "GET",
      });
      const data = await res.json();
      console.log("lineList raw:", data.content);
  ...
  const handleDeactivate = async (delegateId) => {
    ...
    await apiRequest(`/api/approval/management/delegate/${delegateId}`, {
      method: "PATCH",
    });
    setSearchTrigger((prev) => prev + 1);
  };
  ...
  const handleCreate = async () => {
    ...
    await apiRequest("/api/approval/management/delegate", {
      method: "POST",
      body: JSON.stringify({
        approverId: Number(selectedLine?.defaultApprover),
  ...
  // --- 직원 검색 모달 로직 ---
  const fetchEmployees = async (kw = "") => {
    setEmpLoading(true);
    try {
      const params = new URLSearchParams();
      if (kw) params.append("keyword", kw);
      const res = await apiRequest(
        `/api/hr/employees/simple?${params.toString()}`,
        { method: "GET" },
      );
      const data = await res.json();
      setEmployees(data ?? []); // content 아니고 바로 배열
    } finally {
      setEmpLoading(false);
    }
  };
  ...
  const handleSelectDelegate = (employeeId) => {
    const emp = employees.find((e) => e.employeeId === employeeId);
    setForm((prev) => ({
      ...prev,
      delegateId: employeeId,
      delegateName: emp?.name ?? "",
    }));
    setShowEmployeeModal(false);
  };

  useEffect(() => {
    console.log("selectedLine 바뀜:", selectedLine);
  }, [selectedLine]);
  ...
  return (
    <div className="flex flex-col gap-4 p-5">
        ...
        <CardContent className="flex flex-col p-4 gap-4">
          ...
          {showForm && (
            <div className="bg-gray-50 rounded-2xl p-4 flex flex-col gap-3 border border-gray-200">
              <h3 className="text-sm font-semibold text-[#1a2f4e]">
                대리결재 설정
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  ...
                  >
                    ...
                    {lineList.map((l) => (
                      <option key={l.approvalLineId} value={l.approvalLineId}>
                        [{l.documentTypeName}] {l.departmentName ?? "공통"} -{" "}
                        {l.stepOrder}단계 ({l.approverName})
                      </option>
                    ))}
                  </select>
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-500">결재자</label>
                  <div className="px-3 py-2 text-sm text-gray-700">
                    {selectedLine?.approverName ?? "-"}
                  </div>
                </div>

                {/* 대리결재자 - 클릭하면 모달 오픈 */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-500">대리결재자</label>
                  <Input
                    readOnly
                    placeholder="클릭하여 대리결재자 선택"
                ...
                <TableBody>
                  {delegateList.map((d) => (
                    ...
                    >
                      ...
                      <TableCell>{d.approvalLineId}</TableCell>
      ...
      {showEmployeeModal && (
        ...
        >
          <div
            className="w-full max-w-md h-[70vh] bg-white rounded-xl shadow-lg overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <EmployeeSearch
              employees={employees}
              selectedEmployeeId={form.delegateId}
              keyword={empKeyword}
              isLoading={empLoading}
              onKeywordChange={setEmpKeyword}
              onKeywordKeyDown={(e) =>
                e.key === "Enter" && fetchEmployees(empKeyword)
              }
              onSearch={() => fetchEmployees(empKeyword)}
              selectEmployee={handleSelectDelegate}
            />
          </div>
        </div>
      )}
