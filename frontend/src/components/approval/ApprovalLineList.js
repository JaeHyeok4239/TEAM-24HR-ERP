...
export default function ApprovalLineList() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const res = await apiRequest(`/api/approval/line?${params.toString()}`, { method: "GET" });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setTypeList(await res.json());
    };
    load();
  }, []);

  // 부서 API 경로 실제 경로로 수정 필요
  useEffect(() => {
    const load = async () => {
      const res = await apiRequest("/api/hr/reference-data/departments", { method: "GET" });
      ...
      <Card className="transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg">
        <CardContent className="flex flex-col p-4 gap-4">
          ...
          <div className="bg-[#ffffffbd] rounded-lg border-[#94abcaa1] overflow-hidden">
            {lineList.length === 0 ? (
              <p className="text-center text-sm text-gray-400 py-10">결재선이 없습니다</p>
            ) : (
              <Table>
                <TableHeader className="bg-[#94abcaa1]">
                  <TableRow className="text-[#1a2f4e]">
                    ...
                    {isAdmin && <TableHead className="font-bold text-center">관리</TableHead>}
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {lineList.map((line) => (
                    <TableRow key={line.approvalLineId} className="text-center text-[#1a2f4e]">
                      <TableCell>{line.departmentName ?? "공통"}</TableCell>
                      <TableCell>{line.documentTypeName}</TableCell>
                      <TableCell>{line.stepOrder}단계</TableCell>
                      <TableCell>{line.approverName}</TableCell>
                      <TableCell>{line.positionName}</TableCell>
                      {isAdmin && (
                        <TableCell>
                          <div className="flex gap-2 justify-center">
                            <Button size="sm" className="bg-[#ffc23f8c] hover:bg-[#ffae00]">
                              <Pencil size={14} />
                            </Button>
                            <Button size="sm" variant="destructive" onClick={() => handleDelete(line.lineId)}>
                              <Trash2 size={14} />
                            </Button>
                          </div>
                        </TableCell>
