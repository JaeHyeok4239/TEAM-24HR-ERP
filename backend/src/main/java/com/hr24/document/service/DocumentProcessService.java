...
public class DocumentProcessService {
	...
	public DocumentDetailDto.LeaveRequestDto executeLeave(Long documentId) {

	    Document document = documentRepository.findById(documentId)
	            .orElseThrow(() -> new EntityNotFoundException("존재하지 않는 문서입니다"));

	    DocumentDetailDto.LeaveRequestDto dto;

	    try {
	        dto = objectMapper.convertValue(document.getDocumentContent(), DocumentDetailDto.LeaveRequestDto.class);
	...
	// 근태 정정 데이터 생성
	@Transactional
	public DocumentDetailDto.AttendanceCorrectionRequestDto executeCorrection(Long documentId) {

		Document document = documentRepository.findById(documentId)
				.orElseThrow(() -> new EntityNotFoundException("존재하지 않는 문서입니다"));

		DocumentDetailDto.AttendanceCorrectionRequestDto dto;

		try {
			dto = objectMapper.convertValue(document.getDocumentContent(), DocumentDetailDto.AttendanceCorrectionRequestDto.class);
